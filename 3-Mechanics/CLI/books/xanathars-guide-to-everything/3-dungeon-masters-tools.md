...
Falling from a great height is a significant risk for adventurers and their foes. The rule given in the player's handbook is simple: at the end of a fall, you take `dice:1d6|noform|noparens|avg` (`d6`) bludgeoning damage for every 10 feet you fell, to a maximum of `dice:20d6|noform|noparens|avg` (`20d6`). You also land [prone](conditions.md#Prone), unless you somehow avoid taking damage from the fall. Here are two optional rules that expand on that simple rule.
...
A flying creature in flight falls if it is knocked [prone](conditions.md#Prone), if its speed is reduced to 0 feet, or if it otherwise loses the ability to move, unless it can hover or it is being held aloft by magic, such as the [fly](/3-Mechanics/CLI/spells/fly-xphb.md) spell.
...
Tools have more specific applications than skills. The [History](skills.md#History) skill applies to any event in the past. A tool such as a [forgery kit](/3-Mechanics/CLI/items/forgery-kit-xphb.md) is used to make fake objects and little else. Thus, why would a character who has the opportunity to acquire one or the other want to gain a tool proficiency instead of proficiency in a skill?
...
To prepare for using a simple trap in play, start by making note of the characters' passive Wisdom ([Perception](skills.md#Perception)) scores. Most traps allow Wisdom ([Perception](skills.md#Perception)) checks to detect their triggers or other elements that can tip off their presence. If you stop to ask players for this information, they might suspect a hidden danger.
...
A trigger requires a Wisdom ([Perception](skills.md#Perception)) check if simply spotting it reveals its nature. The characters can foil a pit trap hidden by a leaf-covered net if they spot the pit through a gap in the leaves. A trip wire is foiled if it is spotted, as is a pressure plate.

Other traps require careful inspection and deduction to notice. A doorknob opens a door when turned to the left, but activates a trap when turned to the right. Such a subtle trap requires a successful Intelligence ([Investigation](skills.md#Investigation)) check to notice. The trigger is obvious. Understanding its nature is not.
...
The trigger for a complex trap should be as foolproof as you can make it. A complex trap represents a serious expenditure of effort and magical power. No one builds such a trap and makes it easy to avoid. Wisdom ([Perception](skills.md#Perception)) and Intelligence ([Investigation](skills.md#Investigation)) checks might be unable to spot a trigger, especially a magical one, but they can still give hints about the trap before it triggers. Bloodstains, ashes, gouges in the floor, and other clues of that sort can serve as evidence of the trap's presence.
...
Not all of the characters' options need to be focused on stopping a trap from operating. Think of what characters can do to mitigate or avoid a trap's effects. Making the trap vulnerable to this sort of effort is a way to engage characters who might be ill-suited to confront the trap directly. A successful Intelligence ([Religion](skills.md#Religion)) check might provide insight into the imagery displayed by a trap in a temple or shrine, giving other characters a clue about how and where to direct their efforts. A character could stand in front of a dart trap while holding a shield that the darts can target harmlessly, while other characters trigger that element as they work to disable it.
...
A character with the noble background can mingle with the upper class, but other characters can do so only if you judge that the character has made sufficient contacts. Alternatively, a character might use a [disguise kit](/3-Mechanics/CLI/items/disguise-kit-xphb.md) and the [Deception](skills.md#Deception) skill to pass as a noble visiting from a distant city.

##### Resolution

After a workweek of carousing, a character stands to make contacts within the selected social class. The character makes a Charisma ([Persuasion](skills.md#Persuasion)) check using the Carousing table.
...
To complete a magic item, a character also needs whatever tool proficiency is appropriate, as for crafting a nonmagical object, or proficiency in the [Arcana](skills.md#Arcana) skill.
...
To attempt a crime, the character makes three checks: Dexterity ([Stealth](skills.md#Stealth)), Dexterity using [thieves' tools](/3-Mechanics/CLI/items/thieves-tools-xphb.md), and the player's choice of Intelligence ([Investigation](skills.md#Investigation)), Wisdom ([Perception](skills.md#Perception)), or Charisma ([Deception](skills.md#Deception)).
...
The character makes three checks: Wisdom ([Insight](skills.md#Insight)), Charisma ([Deception](skills.md#Deception)), and Charisma ([Intimidation](skills.md#Intimidation)). If the character has proficiency with an appropriate gaming set, that tool proficiency can replace the relevant skill in any of the checks. The DC for each of the checks is `dice:5+2d10|noform|noparens|avg` (`5 + 2d10`); generate a separate DC for each one. Consult the Gambling Results table to see how the character did.
...
At the end of the required time, the character chooses to make either an Intelligence ([Religion](skills.md#Religion)) check or a Charisma ([Persuasion](skills.md#Persuasion)) check. The total of the check determines the benefits of service, as shown on the Religious Service table.
